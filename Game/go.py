...
class Stone(object):
    ...
    def to_str(self):
        """Return the location of the stone, e.g. '[1, 1]'."""
...
class Group(object):
    ...
    def update_liberties(self):
        ...
        if len(self.liberties) == 0:
            self.remove()
        # Print all stones coordinatez on the board
        poss.clear()
        for x in self.stones:
            poss.append(x.to_str())
            # print (x.to_str())

    def request_update_board(self):
        print (poss)
        return poss
