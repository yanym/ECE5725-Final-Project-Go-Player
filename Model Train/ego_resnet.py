...
def conv_block(inputs, depth, block_num, l2_v = 0.000003):
...
def create_model():
    ...
    x = residual_block(x, [16,16,64 ], 2, '2')
    ...
    z = residual_block(z, [32, 32,128], 3, 'B3')
    z = tf.keras.layers.MaxPool2D((4, 4),strides=(2 , 2))(z)
    z = residual_block(z, [64, 64,256], 6, 'B4')
    z = tf.keras.layers.MaxPool2D((2, 2),strides=(2,2))(z)
    z = residual_block(z, [128, 128,512], 3, 'B5')
    z = tf.keras.layers.AveragePooling2D((2, 2))(z)
    z = output_block(z)
    z2 = tf.keras.layers.concatenate([z, ego_input1])
    z2 = tf.keras.layers.Dense(1024, activation='relu',kernel_regularizer = tf.keras.regularizers.l2(l=0.003))(z2)
...
def train_model(model,input0,input1,output0,output1,checkpoint_name,testx0,testx1,testy0,testy1):
    ...
    history = model.fit({'ego_input0': input0, 'ego_input1': input1}, {'ego_output0': output0, 'ego_output1': output1},
                        epochs=8, callbacks= [cb_end, cb_save], batch_size=64,
